...
| F1.STUDY.NEW.MATCH.1 | reviewMode → matchMode transition concept | Docs | In progress → `Done` after merge | [07-study-modes.md → matchMode](../design/07-study-modes.md#matchmode-mode-thứ-2); [06-srs-8box.md](../design/06-srs-8box.md#kích-hoạt-srs-box-0--box-1); [deck-management.md → Play Menu](../design/screens/deck-management.md#play-menu); [new-learning-flow-decision-table.md](../decision-tables/new-learning-flow-decision-table.md) | `npm run check` / `git diff --check` | PR `docs/review-to-match-flow` (squash commit after merge) |
| F1.STUDY.NEW.MATCH.2 | matchMode flow state and evaluation (pairing, correct/wrong, batch completion → guessMode; pre-SRS) | BE | Not started | [07-study-modes.md → matchMode](../design/07-study-modes.md#matchmode-mode-thứ-2) | TBD | TBD |
| F1.STUDY.NEW.MATCH.3 | matchMode UI screen (two sides prompt ↔ meaning; shuffle; correct/wrong feedback) | FE | Not started | [07-study-modes.md → matchMode](../design/07-study-modes.md#matchmode-mode-thứ-2) | TBD | TBD |
...
> `F1.DM.1`, `F1.DM.SORT.1`, `F1.DM.PLAY.1`, `F1.STUDY.NEW.REVIEW.1`, `F1.STUDY.NEW.MATCH.1`, and `F1.STUDY.NEW.GUESS.1` are docs rows: they flip to `Done` once this PR is
