...
| MX-16   | —      | Appearance / Theme Settings group | `appearance-theme-settings.card.html`  | Settings      | Complete only when MX-16.1 to MX-16.5 are done                                  | done   | All children MX-16.1–.5 done: 10 frames (dark+light), 0 render errors, token-lint clean; added `[data-theme="dark"]` selector to kit source so theme-preview swatches force a theme accurately |
| MX-16.1 | MX-16  | Presets default                   | `appearance-theme-settings.card.html`  | Settings      | system/dark/light presets default                                               | done   | Three `MxThemePresetTile` (System split-preview / Dark / Light), System selected by default + explainer callout; dark+light verified |
| MX-16.2 | MX-16  | Selected theme                    | `appearance-theme-settings.card.html`  | Settings      | selected system/dark/light states                                               | done   | Selected ring on Dark (dark frame) / Light (light frame); System-selected covered by 16.1 — all three selection states shown; dark+light verified |
| MX-16.3 | MX-16  | App icon states                   | `appearance-theme-settings.card.html`  | Settings      | selected + unavailable app icon states                                          | done   | Icon tiles Classic (selected ring+check) / Mint / Sunset (`unavailable`, dimmed + Soon) + explainer; selected ≠ unavailable; dark+light verified |
| MX-16.4 | MX-16  | Live preview                      | `appearance-theme-settings.card.html`  | Settings      | live theme preview dark + light                                                 | done   | "Live preview" section shows a deck card sample (cover, badges) rendered in the current theme; dark+light verified |
| MX-16.5 | MX-16  | Save failed                       | `appearance-theme-settings.card.html`  | Settings      | save failed state                                                               | done   | `MxErrorState` "current theme still works … settings are safe on this device" + Retry; dark+light verified |
| MX-17   | —      | App Settings group                | `app-settings.card.html`               | Settings      | Complete only when MX-17.1 to MX-17.6 are done                                  | done   | All children MX-17.1–.6 done: 12 frames (dark+light), 0 render errors, token-lint clean; backup/restore/cloud via MxSettingsSection/Row/Switch |
| MX-17.1 | MX-17  | Loaded                            | `app-settings.card.html`               | Settings      | loaded App Settings in dark + light                                             | done   | Backup / Restore (high-risk) / Sync / About sections; Back-up action, Auto-backup switch, version/storage; dark+light verified |
| MX-17.2 | MX-17  | Backup states                     | `app-settings.card.html`               | Settings      | backup idle/in-progress/success/failed                                          | done   | Gallery: idle (button) / in-progress (loading 60%) / success (green Done badge) / failed (red + triangle + "data is safe" + Retry); success=green≠failed=red; dark+light verified |
| MX-17.3 | MX-17  | Restore confirm                   | `app-settings.card.html`               | Settings      | high-risk overwrite warning confirm                                             | done   | `MxConfirmDialog` highRisk "Restore from backup?" — explicit overwrite warning ("added since … overwritten, can't be recovered"), "Overwrite & restore"; dark+light verified |
| MX-17.4 | MX-17  | Restore failed                    | `app-settings.card.html`               | Settings      | restore failed + data-safe/retry copy                                           | done   | `MxErrorState` "Restore didn't finish — nothing changed, current data intact and safe" + Try again; dark+light verified |
| MX-17.5 | MX-17  | Cloud sync                        | `app-settings.card.html`               | Settings      | cloud-sync soon/blocked state                                                   | done   | `MxSettingsRow unavailable` (Cloud sync) + "coming soon" unavailable callout ("local-first, use Backup for now"); dark+light verified |
| MX-17.6 | MX-17  | Save failed                       | `app-settings.card.html`               | Settings      | save failed state                                                               | done   | `MxErrorState` "Couldn't save that setting — existing settings and data are safe" + Retry; dark+light verified |
| MX-18   | —      | Native Language Picker group      | `native-language-picker.card.html`     | List/Form     | Complete only when MX-18.1 to MX-18.6 are done                                  | done   | All children MX-18.1–.6 done: 16 frames (dark+light), 0 render errors, token-lint clean; MxLanguageRow list, selected≠correct, search-empty≠error |
| MX-18.1 | MX-18  | Loaded list                       | `native-language-picker.card.html`     | List/Form     | language list in dark + light                                                   | done   | `MxLanguageRow` list (native + English name + flag/code avatar), search field, English selected; dark+light verified |
| MX-18.2 | MX-18  | Loading                           | `native-language-picker.card.html`     | List/Form     | loading state                                                                   | done   | 6 row skeletons (avatar + name lines) on `--mx-color-state-loadingSurface`; dark+light verified |
| MX-18.3 | MX-18  | Selected current                  | `native-language-picker.card.html`     | List/Form     | selected current language                                                       | done   | "Current" section pins the selected language (Vietnamese, purple check — **not** study-green) above "All languages"; dark+light verified |
| MX-18.4 | MX-18  | Search active                     | `native-language-picker.card.html`     | List/Form     | search active                                                                   | done   | Query "ja" → filtered result + count caption + clear button; dark+light verified |
| MX-18.5 | MX-18  | Search empty / long scroll        | `native-language-picker.card.html`     | List/Form     | search-empty + long-list scroll                                                 | done   | Two variants: `MxEmptyState` (neutral, **not** error) "No languages match" + Clear; 18-language long list scrolling in `.mx-scroll`; dark+light verified |
| MX-18.6 | MX-18  | Save failed/error                 | `native-language-picker.card.html`     | List/Form     | save failed + load error                                                        | done   | Save-failed destructive callout "previous language still active, data safe" + list; `MxErrorState` load error "current language still works, data safe" + Retry; dark+light verified |
| MX-19   | —      | Reminder Settings group           | `reminder-settings.card.html`          | Form          | Complete only when MX-19.1 to MX-19.7 are done                                  | done   | All children MX-19.1–.7 done: 18 frames (dark+light), 0 render errors, token-lint clean; MxReminderRow / MxWeekdayChip form |
| MX-19.1 | MX-19  | Enabled/disabled                  | `reminder-settings.card.html`          | Form          | reminder enabled/disabled states                                                | done   | Master switch + `MxReminderRow` enabled (13:00 Mon–Fri) and disabled (20:00, dimmed); dark+light verified |
| MX-19.2 | MX-19  | Weekday chips                     | `reminder-settings.card.html`          | Form          | selected/unselected chips                                                       | done   | `MxWeekdayChip` row (Mon–Fri selected purple / Sat–Sun muted) + in-reminder chips; selected ≠ unselected; dark+light verified |
| MX-19.3 | MX-19  | Time picker                       | `reminder-settings.card.html`          | Form          | time picker state                                                               | done   | `MxModalSheet` "Set reminder time" — hour/minute picker columns with selected value highlighted + "Set 13:00"; dark+light verified |
| MX-19.4 | MX-19  | Add reminder                      | `reminder-settings.card.html`          | Form          | add reminder state                                                              | done   | "Add reminder" CTA + newly-added 09:00 row + info callout; dark+light verified |
| MX-19.5 | MX-19  | Keyboard-safe input               | `reminder-settings.card.html`          | Form          | keyboard-safe form layout                                                       | done   | Focused label input ("Morning Korean" + caret) above a faux QWERTY keyboard (safe-area inset); dark+light verified |
| MX-19.6 | MX-19  | Permission needed/blocked         | `reminder-settings.card.html`          | Form          | permission-needed + blocked state                                               | done   | Needed: warning callout + "Allow notifications"; Blocked: unavailable callout + "Open system settings" (guidance, not a system error); dark+light verified |
| MX-19.7 | MX-19  | Save failed/delete confirm        | `reminder-settings.card.html`          | Form          | save failed + delete confirm                                                    | done   | Save-failed destructive callout "existing reminders still work, settings safe" + Retry; `MxConfirmDialog` destructive "Delete this reminder?"; dark+light verified |
| MX-20   | —      | Spaced Repetition Settings group  | `spaced-repetition-settings.card.html` | Settings/Form | Complete only when MX-20.1 to MX-20.6 are done                                  | done   | All children MX-20.1–.6 done: 14 frames (dark+light), 0 render errors, token-lint clean; box count kept as an OPEN QUESTION (8-box source of truth honored) |
| MX-20.1 | MX-20  | Loaded                            | `spaced-repetition-settings.card.html` | Settings/Form | loaded settings in dark + light                                                 | done   | Boxes (8-box fixed) / Review behavior (repeat order, step-down lapse, due notifications) / Game source / Reset sections; dark+light verified |
| MX-20.2 | MX-20  | Box-count open question           | `spaced-repetition-settings.card.html` | Settings/Form | "Ô: 7" shown as OPEN question, not final 7-box rule                             | done   | Box-count "7 · Under review" (disabled) + warning callout "Open question — not final: MemoX is 8-box, no 8→7 migration implied"; does **not** assert 7-box; dark+light verified |
| MX-20.3 | MX-20  | Interval rows                     | `spaced-repetition-settings.card.html` | Settings/Form | interval row states                                                             | done   | 8 box-interval rows (Box 1..8 with default gaps) — reinforces the 8-box model; dark+light verified |
| MX-20.4 | MX-20  | Warning                           | `spaced-repetition-settings.card.html` | Settings/Form | SRS warning/callout state                                                       | done   | Amber `MxCallout` "Changes apply to future reviews only — current due dates/boxes unchanged" (warning, not error); dark+light verified |
| MX-20.5 | MX-20  | Reset confirm                     | `spaced-repetition-settings.card.html` | Settings/Form | reset-to-default destructive confirm                                            | done   | `MxConfirmDialog` destructive "Reset SRS settings to default?" — clarifies words/boxes/due dates unaffected, only preferences reset; dark+light verified |
| MX-20.6 | MX-20  | Unavailable/save failed           | `spaced-repetition-settings.card.html` | Settings/Form | unavailable/future + save failed                                                | done   | Unavailable rows (notifications / random order, Soon) + unavailable callout; `MxErrorState` "previous preferences still active, words/boxes/due safe" + Retry; dark+light verified |
