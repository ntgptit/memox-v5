...
- Bấm **Cài đặt** → mở **[App Settings](app-settings.md)** (cấu hình chính; **khác** Chủ đề /
  Appearance). Chứa: **Tiếng mẹ đẻ**, **Lời nhắc**, **Hình thức từ ngữ**, **Lặp lại giãn cách**, **Cài
  đặt trò chơi**, **Sao lưu/Khôi phục**, **Đồng bộ đám mây**; + learning preferences (SRS intervals,
  lapse rule, thẻ mới/ngày — [09-settings](../09-settings.md)) và **ngôn ngữ UI** ([08-i18n](../08-i18n.md)).
- Child screen/flow của App Settings:
  - **Tiếng mẹ đẻ** → [Native Language Picker](native-language-picker.md).
  - **Lời nhắc** → [Reminder Settings](reminder-settings.md).
  - **Lặp lại giãn cách** → [Spaced Repetition Settings](spaced-repetition-settings.md).
- **Backup/Restore** và **Cloud Sync** là **high-risk/data features** cần **policy** trước implementation
  (restore = destructive; sync = conflict policy). **Drawer không** phải UI bắt buộc.
